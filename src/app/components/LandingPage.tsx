...
export default function LandingPage() {
  ...
  useEffect(() => {
    ...
    const serviceInterval = window.setInterval(() => {
      setActiveService((prev) => (prev + 1) % totalServices);
    }, 4000);
  ...
  function handleServiceContact(service: Service) {
    const mailtoLink = `mailto:${contactInfo.email}?subject=${encodeURIComponent(service.mailSubject)}&body=${encodeURIComponent(
      service.mailBody,
    )}`;
    window.location.href = mailtoLink;
  }

  function handlePlanContact(plan: Plan) {
    const mailSubject = `Consulta: Plan ${plan.title}`;
    const mailBody = `Hola Benjamín,\n\nEstoy interesado en el plan ${plan.title}.\n\nDetalles de mi proyecto:\n[Describe tu proyecto aquí]\n\nSaludos.`;
    const mailtoLink = `mailto:${contactInfo.email}?subject=${encodeURIComponent(mailSubject)}&body=${encodeURIComponent(mailBody)}`;
    window.location.href = mailtoLink;
  }
        ...
        <div className="mx-auto flex max-w-7xl items-center justify-between px-4 py-4 sm:px-6 lg:px-8">
          <div className="flex items-center gap-3">
            <div className="relative">
              <div className="absolute -top-0.5 -right-0.5 h-2 w-2 animate-ping rounded-full bg-cyan-400" />
              <span className="bg-gradient-to-r from-cyan-400 to-blue-500 bg-clip-text text-sm font-semibold uppercase tracking-[0.3em] text-transparent">
                {contactInfo.name}
              </span>
            </div>
          </div>
          <nav className="hidden items-center gap-6 text-sm md:flex">
            {navLinks.map(({ label, href, external }) => (
              <a
                key={href}
                href={href}
                onClick={external ? undefined : handleNavClick(href)}
                className="group relative text-neutral-400 transition-all duration-300 hover:text-white"
                {...(external
                  ? {
            ...
            <a
              href="#contacto"
              className="group relative overflow-hidden rounded-full bg-gradient-to-r from-cyan-500 to-blue-600 px-6 py-2 font-semibold text-white transition-all duration-300 hover:scale-105 hover:shadow-2xl hover:shadow-cyan-500/25"
      ...
      <main className="relative z-10 mx-auto max-w-7xl px-4 pb-24 pt-[72px] sm:px-6 sm:pt-[80px] lg:px-8">
        <section className="relative flex flex-col gap-12 py-20 lg:min-h-[80vh] lg:flex-row lg:items-center">
          ...
          <div className="relative z-10 lg:w-3/5">
            <div className="mb-6 inline-flex items-center gap-2 rounded-full border border-cyan-500/30 bg-cyan-500/10 px-4 py-2 text-xs uppercase tracking-[0.3em] text-cyan-300 backdrop-blur-sm">
              <Rocket className="h-3 w-3" />
              {contactInfo.role} · {contactInfo.location.split(',')[0]}, ARG
            </div>
            <h1 className="text-5xl font-bold leading-tight text-transparent sm:text-6xl">
              <span className="bg-gradient-to-br from-white via-cyan-100 to-blue-200 bg-clip-text">
                {contactInfo.tagline}
              </span>
            </h1>
            <p className="mt-6 max-w-2xl text-xl leading-relaxed text-neutral-300">{contactInfo.description}</p>
            <div className="mt-8 flex flex-col gap-4 sm:flex-row sm:flex-wrap">
              <a
                href="#contacto"
                className="group relative inline-flex items-center justify-center rounded-full bg-gradient-to-r from-cyan-500 to-blue-600 px-8 py-4 font-semibold text-white shadow-2xl shadow-cyan-500/20 transition-all duration-300 hover:scale-105 hover:from-cyan-400 hover:to-blue-500 hover:shadow-cyan-500/40"
                onClick={handleNavClick('#contacto')}
              >
              ...
              <a
                href="#casos"
                className="group inline-flex items-center justify-center rounded-full border border-neutral-700 bg-neutral-900/50 px-8 py-4 font-semibold text-neutral-200 backdrop-blur transition-all duration-300 hover:border-cyan-500/50 hover:bg-neutral-800/50 hover:text-white"
          ...
          <div className="grid flex-1 gap-6 sm:grid-cols-3 lg:grid-cols-1 lg:gap-6">
            {stats.map(({ value, label, Icon }, index) => (
              <div
                key={label}
                className="group relative rounded-2xl border border-neutral-800 bg-neutral-900/30 p-6 backdrop-blur transition-all duration-500 hover:scale-105 hover:border-cyan-500/30 hover:bg-neutral-900/50 hover:shadow-2xl hover:shadow-cyan-500/10"
              ...
              >
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <div className="text-2xl font-extrabold text-cyan-400">{value}</div>
